...
if echo "$answer" | grep -iq "^y" ;then
    ...
    if echo "$answer" | grep -iq "^y" ;then
      ...
      echo -e "\nDaemon will not be started now.\n"
    fi
else
...
##fi
rm -Rf $HOME/$TEMPDIR
echo -e "If this installation was completed as a masternode installation please allow some time for the service to update its database then you can try to activate your masternode on your local wallet and use command << $COINCLI masternode status >> on your server to verify that it have been started. You can also verify amount of blocks synced by using command << $COINCLI getinfo >>\n\n"
