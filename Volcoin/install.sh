...
USER=voluser
TEMPDIR=tempVOL
COINGIT="https://github.com/volcoin/volcoin"
COINGITSENTINEL=""
COIN=VolCoin
COIND=volcoind
COINDS=volcoind_screen
COINCLI=""
COINCONF=VolCoin.conf
COINCORE=.VolCoin
COINS="VOL:"
COINDEV="vjpHdDAr8ZcRzGBQxHtvXvnGYJhcpnKJdK"
ADDCOINCONF=""
...
if echo "$answer" | grep -iq "^y" ;then
  echo -e "\nOK.\n"
  mkdir $HOME/$COIN
  chmod -R 777 $HOME/$COIN
  sudo git clone $COINGIT $HOME/$COIN
  cd $HOME/$COIN/src
  sudo make -f makefile.unix
  sudo make install
  cp volcoind /usr/local/bin/
  cd $HOME
  mkdir $HOME/$COINCORE
